...
function getAssessmentNumber(weekNumber, assessmentId) {
    ...
    const assessment = week?.tasks?.assessments?.find(a => a.id === assessmentId);
    if (assessment && typeof assessment.displayNumber === 'number') {
        return assessment.displayNumber;
    }
    ...
    for (let i = 0; i < weeks.length; i++) {
        ...
        if (w.number < weekNumber) {
            count += w.tasks.assessments.length;
        } else if (w.number === weekNumber) {
            const assessmentIndex = w.tasks.assessments.findIndex(a => a.id === assessmentId);
                ...
                <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-color);">
                    ...
                    <div>
                        ...
                        ${Object.entries(week.schedule.assessments).map(([id, dueDate]) => {
                            ...
                            const when = assessment.inClass
                                ? (assessment.inClassIncludeTime ? `${formatDateTime(dueDate)} (in-class)` : `${formatDate(dueDate)} (in-class)`)
    ...
    if (week.tasks.assessments.length > 0) {
        ...
        week.tasks.assessments.forEach(assessment => {
            ...
            const dueDate = assessmentDueRaw
                ? (assessment.inClass && !assessment.inClassIncludeTime ? formatDate(assessmentDueRaw) : formatDateTime(assessmentDueRaw))
                : null;
